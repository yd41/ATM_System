...
public class INSERT {
  ...
  static   public void Insert(String account, String password, Double balance, String username, String phoneNum, String gender) {
    ...
    public class Info {
      ...
      static  public final String [][] numList={
                //移动电话号码前三位
                {
                    "134","135","136",
                    "137","138","139",
     ...
     static   public String GetNum(){
            Random random = new Random(System.currentTimeMillis());

            String num="";
            int i1 = random.nextInt(numList.length);
            num+=numList[i1][random.nextInt(numList[i1].length)];
            for (int i = 0; i < 8; i++) {
...
return num;
    ...
    public static void main(String[] args) throws SQLException, ClassNotFoundException {
        ...
        Init();
        String password=String.valueOf("123456".hashCode());

        for (int i = 0; i < 100; i++) {
            String account=Info.GetAccount();
            String gender=random.nextBoolean()?"男":"女";
            String phoneNum=Info.GetNum();
            String username=Info.getRandomUserName();
            double balance=Double.parseDouble(String.format("%.2f",Math.abs(random.nextDouble(10000))));
            if(!Query(account)){
                Insert(account,password,balance,username,phoneNum,gender);
            }
